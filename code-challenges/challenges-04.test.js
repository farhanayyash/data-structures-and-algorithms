...
// console.log(containsW("hello sworld"));
...
// console.log(containsWorld("wodrldd"));
/* ------------------------------------------------------------------------------------------------
CHALLENGE 5
...
const isCapitalized = (str) => {
  ...
  local.forEach(element => {
    let x = element[0];
    let checkx = element.length;
    let checkxxx = 0;
    if (x.charCodeAt(0) <= 90 && x.charCodeAt(0) >= 65) {

      [...element].forEach(element2 => {
        if (element2 == "," || element2 == "." || element2 == ")" || element2 == "?" || element2 == "!") {
          checkxxx += 1;
        }
      });
      localarray.push(element.slice(0, checkx - checkxxx));
    }
  });
  return localarray;
};
let xx ='We only want to Return the Words that begin With a capital Letter';

let xxx ='Given by our hand in the meadow that is called Runnymede, between Windsor and Staines, on the fifteenth day of June in the seventeenth year of our reign (i.e. 1215: the new regnal year began on 28 May).';



const isCapitalized2 = (str) => {
  let regex = /[A-Z]\w+/g;
  let localarray = str.match(regex);
  return localarray;
};

console.log(isCapitalized2(xx));
console.log(isCapitalized(xx));

console.log(isCapitalized2(xxx));
console.log(isCapitalized(xxx));
...
let cities2 = ['Albuquerque', 'Chicago', 'Philadelphia', 'Newark', 'Sacramento', 'Eugene'];
console.log(citiesAtoJ(cities2));
