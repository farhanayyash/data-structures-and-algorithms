...
const listFoods = (recipe) => {
  ...
  for(let i = 0 ; i < recipe.ingredients.length ; i ++){
    let count1 =recipe.ingredients[i].indexOf(" ");
    let ar1 = recipe.ingredients[i].slice(count1+1,recipe.ingredients[i].length)

    let count2 =ar1.indexOf(" ");
   
    let ar2 = ar1.slice(count2+1,ar1.length)
    result.push(ar2);
  }
  return result;
};
