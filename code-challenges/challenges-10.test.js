...
const getNames = (arr) => {
  let y = []
  let arrnew = arr.map(myfunction);
  function myfunction(x){
    y.push(x.name.split("").reverse().join(""));
    return x;
  }
  return y;
};
console.log(getNames(arrr));
/* ------------------------------------------------------------------------------------------------
CHALLENGE 2
...
const count = (target, input) => {
  ...
  input.forEach(element => {
    let sum = element.reduce(function (accumulator, currentValue) {
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  input.forEach(element => {
    let x = [];
    let sum = element.filter(function(i) {
...
console.log(divisibleByFiveTwoToThePower([ [0,2,5,4], [2,4,10], [] ]));
/* ------------------------------------------------------------------------------------------------
CHALLENGE 5 
...
let findShortest = (data) => {
  ...
  let sum = data.reduce(function (accumulator, currentValue) {
    if(accumulator> parseInt(currentValue.height)){
      x = currentValue.name;
      return parseInt(currentValue.height);
    }else{
      return accumulator;
    }
}, 200);
